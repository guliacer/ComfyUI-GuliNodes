...
- 基础图像处理：文件夹图片序列加载、RGBA 转 RGB、尺寸调整、裁剪、翻转旋转、亮度/对比度/饱和度/锐化/虚化、色彩校正。
...
| `GG 图像CAS锐化+` | CAS 锐化增强。 |
| `GG 图像文件夹加载` | 选择本地图片文件夹，在节点内预览、勾选并拖拽排序，每次运行按顺序输出下一张图片。 |
| `GG 图像预览` | 预览图像。 |
| `GG 图像保存` | 保存图像。 |
| `GG 图像压缩保存` | 压缩并保存图像。 |
| `GG 图像压缩` | 输出压缩后的图像。 |
...
- 新增 `GG 图像文件夹加载` 节点：可选择本地图片文件夹，在节点内预览图片、点击选择、拖拽排序，并在每次运行工作流时按顺序输出下一张图片。
